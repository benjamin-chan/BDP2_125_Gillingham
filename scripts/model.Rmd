...
```{r}
conditions <- M %>% tidy(effects = "fixed") %>% pull(term) %>% grep("^cond", ., value = TRUE) %>% grep(":", ., invert = TRUE, value = TRUE)
metabolites <- M %>% tidy(effects = "fixed") %>% pull(term) %>% grep("^meta", ., value = TRUE)
contrasts <- 
  expand.grid(condition = conditions, metabolite = metabolites) %>%
  mutate(contrast = sprintf("%s:%s + %s = 0", condition, metabolite, condition)) %>%
  pull(contrast)
FTests <-
  lapply(contrasts, FUN = testContrast, nlmeObj = M) %>%
  unlist() %>%
  matrix(nrow = length(contrasts), byrow = TRUE) %>%
