...
D2 <- D2 %>% filter(!(metabolite == "FUMARIC" & value > 30))
```


## Check data

Check the `value` and `zValue`.

```{r plotDataAim1, results = "asis", fig.height = 6, fig.width = 10}
kable(summarizeOutcome(D1))
ggplot(D1) +
  aes(x = zValue, y = metabolite, color = activity, fill = activity) +
  geom_jitter(alpha = 1/2) +
  facet_wrap(~ genotype) +
  scale_x_continuous("Z-score of normalized values") +
...
ggplot(D2) +
  aes(x = zValue, y = metabolite, color = chow, fill = chow) +
  geom_jitter(alpha = 1/2) +
  facet_wrap(~ genotype) +
  scale_x_continuous("Z-score of normalized values") +
  scale_y_discrete("Metabolite") +
